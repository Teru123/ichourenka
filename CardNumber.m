...
-(BOOL)checkColumnExists
{
    ...
    const char *sqlStatement = "select memorised from cardNumberInfo";
    if(openDatabaseResult == SQLITE_OK){
...
-(void) alterDB{
    ...
    if(sqlite3_open([databasePath UTF8String], &sqlite3Database) == SQLITE_OK)
    {
        ...
        NSString *updateSQL = [NSString stringWithFormat: @"ALTER TABLE cardNumberInfo ADD COLUMN memorised integer DEFAULT 0"];
        //const char *update_stmt = [updateSQL UTF8String];
        //sqlite3_prepare_v2(sqlite3Database, update_stmt, -1, &statement, NULL);
        
        [self runQuery:[updateSQL UTF8String] isQueryExecutable:YES];
